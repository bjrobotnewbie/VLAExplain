...
from src.unifolm_vla.model.framework.collector import ATTENTION_TRACER
...
def get_action_state(observations: deque, task_description: str, model: Unifolm_VLA_Inference):
    ...
    if ATTENTION_TRACER.is_collect_raw_images:
        for i, observation in enumerate(observations):
            ATTENTION_TRACER.update_images(observation['full_image'], str(i+1), 'image1')
            ATTENTION_TRACER.update_images(observation['wrist_image'], str(i+1), 'image2')
    ...
    ATTENTION_TRACER.update_language_info(
        {
            'text_token_ids': qwen_inputs['input_ids'],
...
def eval_libero(args: Args) -> None:
    ...
    if args.task_id != -1:
        ...
        logger.info(f"Evaluating on single task ID: {args.task_id}")
    else:

...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def forward(
        ...
        # todo: collect infor for attention visulization
        if ATTENTION_TRACER.is_collect_language_attn:
            # deep copy attn_weights
            attn_weights_language = attn_weights.view(bsz, self.num_heads, q_len, key_states.shape[-2]).clone()
            if query_states.dtype == torch.float16:
                attn_weights_language = torch.where(torch.isinf(attn_weights_language), torch.zeros_like(attn_weights_language), attn_weights_language)
            attn_weights_language = nn.functional.softmax(attn_weights_language, dim=-1, dtype=torch.float32).to(query_states.dtype)
            ATTENTION_TRACER.update_language_attention(self.layer_idx, attn_weights_language)

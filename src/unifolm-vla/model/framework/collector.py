...
class AttentionTracer:
    ...
    def __init__(self):
        ...
        self.set_main_dir(os.getenv("VLA_DATA_DIR"))
    ...
    def reset(self):
        ...
        self.current_lan_attn_layer_idx = -1
        self.current_expert_attn_layer_idx = -1
    ...
    def update_lan_attn_layer_idx(self, layer_idx: int):
        """Update language attention layer index"""
        self.current_lan_attn_layer_idx = layer_idx
        if self.current_step not in self.language_attn:
            self.language_attn[self.current_step] = {}
        if self.current_lan_attn_layer_idx not in self.language_attn[self.current_step]:
            self.language_attn[self.current_step][self.current_lan_attn_layer_idx] = {}
    ...
    def update_expert_attn_layer_idx(self, layer_idx: int):
        """Update expert attention layer index"""
        self.current_expert_attn_layer_idx = layer_idx
        if self.current_step not in self.expert_attn:
            self.expert_attn[self.current_step] = {}
        if self.current_time_step not in self.expert_attn[self.current_step]:
            self.expert_attn[self.current_step][self.current_time_step] = {}
        if self.current_expert_attn_layer_idx not in self.expert_attn[self.current_step][self.current_time_step]:
            self.expert_attn[self.current_step][self.current_time_step][self.current_expert_attn_layer_idx] = {}

    def update_images(self, images: torch.Tensor, win_num: str = '1', direction: str='1'):
        """Update images"""
        try:
            # Convert tensor/numpy to numpy array and save
            ...
            if len(img_array.shape) == 4:
                # Batch dimension present: (B, H, W, C) or (B, C, H, W)
                img_array = img_array[0]  # Take first batch element
            
            # Check if transpose is needed
            if len(img_array.shape) == 3:
                if img_array.shape[0] == 3:
                    # Format is (C, H, W) - need to transpose to (H, W, C)
                    img_array = img_array.transpose(1, 2, 0)
                # else: Format is already (H, W, C) - no transpose needed
            ...
            img.save(self.raw_image_dir / f"step_{self.current_step:04d}_{win_num}_{direction}.jpg")
        except Exception as e:
            logger.error(f"Error saving image for step {self.current_step}, win_num {win_num}, direction {direction}: {e}")
    ...
    def update_expert_attention(self, attn_weights: torch.Tensor, layer_idx: int=-1, 
                               device: torch.device = torch.device('cpu')):
        ...
        if layer_idx == -1:
            layer_idx = self.current_expert_attn_layer_idx
        ...
        for image_path in self.raw_image_dir.glob("*.jpg"):
            try:
                ...
                if len(parts) >= 3:
                    direction = parts[3]
                    step = int(parts[1])
                    win_num = int(parts[2]) if len(parts) >= 4 else None
                    
                    if step not in self.raw_images:
                        self.raw_images[step] = {}
                    if win_num is not None:
                        if win_num not in self.raw_images[step]:
                            self.raw_images[step][win_num] = {}
                        self.raw_images[step][win_num][direction] = image
                    else:

...
def _compute_attention_between_modules(
    ...
    for src in source_modules:
        src_q_start, src_q_end = cache_segment_indices[src]
...
def compute_fine_grained_attention(
    ...
    - Image to text/state attention
    - Text/state to image attention
    - Bidirectional text-state attention
    ...
    # Text ↔ State bidirectional attention
    text_to_state = extracted_atten["text_q"][
        :, :, cache_segment_indices["state"][0] : cache_segment_indices["state"][1]
    ]
    state_to_text = extracted_atten["state_q"][
        :, :, cache_segment_indices["text"][0] : cache_segment_indices["text"][1]
    ]

    return {
        **image_to_cross,
        **cross_to_image,
        "text_to_state": text_to_state,
...
def compute_global_attention(
    ...
    - Global image to text/state attention
    - Global text/state to image attention
    - Global bidirectional text-state attention
    ...
    # Global Image → Text/State attention
    image_to_cross = _compute_attention_between_modules(
        extracted_atten,
    ...
    # Global Text/State → Image attention
    cross_to_image = _compute_attention_between_modules(
        extracted_atten,
        Settings.MODULES_CROSS,
        Settings.MODULES_IMAGE,
    ...
    # Global Text ↔ State attention
    text_global_q = avg_pool_attention(extracted_atten["text_q"], dim=1)
    state_global_q = avg_pool_attention(extracted_atten["state_q"], dim=1)

    global_text_to_state = text_global_q[
        :, :, cache_segment_indices["state"][0] : cache_segment_indices["state"][1]
    ]
    global_state_to_text = state_global_q[
        :, :, cache_segment_indices["text"][0] : cache_segment_indices["text"][1]
    ]
    ...
    return {
        **image_to_cross,
        **cross_to_image,
        "global_text_to_state": global_text_to_state,
        "global_state_to_text": global_state_to_text,
    }

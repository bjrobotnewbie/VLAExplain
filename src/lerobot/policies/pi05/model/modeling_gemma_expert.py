...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # collect info
        try:
            if (hasattr(ATTENTION_TRACER, 'is_collect_expert_attn') and 
                ATTENTION_TRACER.is_collect_expert_attn):
                
                if ATTENTION_TRACER.current_step not in ATTENTION_TRACER.expert_attn:
                    ATTENTION_TRACER.expert_attn[ATTENTION_TRACER.current_step] = {}
                
                attn_weights_cpu = attn_weights.detach().cpu().clone()
                ATTENTION_TRACER.update_expert_attention(
                    self.layer_idx, 
                    attn_weights_cpu
                )
        except Exception as e:
            logger.warning(f"Failed to collect attention weights for layer {self.layer_idx}: {e}")

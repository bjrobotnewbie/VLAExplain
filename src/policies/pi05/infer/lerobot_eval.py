...
class ImageProcessor:
    @staticmethod
    def process_image(image: np.ndarray, if_flip=True) -> torch.Tensor:
        if isinstance(image, np.ndarray):
            if image.dtype == np.uint8:
                tensor = torch.from_numpy(image).permute(2, 0, 1).float() / 255.0
            elif image.dtype in [np.float32, np.float64]:
                tensor = torch.from_numpy(image).permute(2, 0, 1).clamp(0, 1)
...
def rollout(
    ...
    while not np.all(done) and step < max_steps:
        ...
        # collect raw images
        if ATTENTION_TRACER.is_collect_raw_images and step < max_steps:
            for key in ["image", "image2"]:
                if "pixels" in obs_orig and key in obs_orig["pixels"]:
                    img = obs_orig["pixels"][key][0]
                    img_tensor = image_processor.process_image(img)
                    if img_tensor is not None and img_tensor.shape[0] == 3:
                        key = 'image1' if key == 'image' else key
                        ATTENTION_TRACER.update_images(key, img_tensor)
        # collect language info
        text_tokens = observation['observation.language.tokens']
        ...
        # collect attention
        if ATTENTION_TRACER.is_collect_expert_attn:
            ATTENTION_TRACER.save_expert_attention()
        if ATTENTION_TRACER.is_collect_language_attn:
